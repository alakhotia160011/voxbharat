// WhatsApp reminder sender — uses Twilio to send WhatsApp messages before campaign calls
import {
...
export class WhatsAppSender {
  ...
  async sendBatch(campaignId, messageTemplate, templateVars = {}) {
    // Create reminder rows from campaign_numbers
    ...
    while ((batch = await getNextPendingReminders(campaignId, 10)) && batch.length > 0) {
      for (const reminder of batch) {
        ...
        try {
          ...
          console.error(`[WhatsApp] Failed to send to ${reminder.phone_number}:`, err.message);
    ...
    console.log(`[WhatsApp] Campaign ${campaignId}: ${sent} sent, ${failed} failed out of ${total}`);
  ...
  async sendSingleReminder(campaignId, campaignNumberId, phoneNumber, messageTemplate, templateVars = {}) {
    // Insert a reminder row for this specific number
    ...
    try {
      ...
      console.log(`[WhatsApp] Sent reminder to ${phoneNumber} (campaign ${campaignId})`);
      return true;
    } catch (err) {
